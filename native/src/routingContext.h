...
struct RoutingContext
{
    ...
    RoutingContext(SHARED_PTR<RoutingConfiguration> config, RouteCalculationMode calcMode = RouteCalculationMode::NORMAL)
        : calculationMode(calcMode), config(config), leftSideNavigation(false), startTransportStop(false), targetTransportStop(false), publicTransport(false), geocoding(false), conditionalTime(0), progress(new RouteCalculationProgress()), precalcRoute(new PrecalculatedRouteDirection())
    {

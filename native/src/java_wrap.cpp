...
jfieldID jfield_GeneralRouter_shortWaySharpTurn = NULL;
jfieldID jfield_GeneralRouter_slightTurn = NULL;
jfieldID jfield_GeneralRouter_shortWaySlightTurn = NULL;
jfieldID jfield_GeneralRouter_roundaboutTurn = NULL;
jfieldID jfield_GeneralRouter_shortWayRoundaboutTurn = NULL;
...
void loadJniRenderingContext(JNIEnv* env) {
	...
	jfield_GeneralRouter_shortWaySlightTurn = getFid(env, jclass_GeneralRouter, "shortWaySlightTurn", "F");
	jfield_GeneralRouter_roundaboutTurn = getFid(env, jclass_GeneralRouter, "roundaboutTurn", "F");
	jfield_GeneralRouter_shortWayRoundaboutTurn = getFid(env, jclass_GeneralRouter, "shortWayRoundaboutTurn", "F");
   ...
   public:
	...
	virtual bool isCancelled() override {
		...
		if (ienv->GetBooleanField(progress, jfield_RouteCalculationProgress_isCancelled)) {
			OsmAnd::LogPrintf(OsmAnd::LogSeverityLevel::Info, "XXX isCancelled = true");
		}
		return ienv->GetBooleanField(progress, jfield_RouteCalculationProgress_isCancelled);
	}

	virtual bool hasMissingMapsNow() override {
		if (progress == NULL) {
			return false;
		}
		if (ienv->GetBooleanField(progress, jfield_RouteCalculationProgress_hasMissingMapsNow)) {
			OsmAnd::LogPrintf(OsmAnd::LogSeverityLevel::Info, "XXX hasMissingMapsNow = true");
		}
...
void parseRouteConfiguration(JNIEnv* ienv, SHARED_PTR<RoutingConfiguration> rConfig, jobject jRouteConfig) {
	...
	rConfig->router->shortWaySharpTurn = ienv->GetFloatField(router, jfield_GeneralRouter_shortWaySharpTurn);
	rConfig->router->slightTurn = ienv->GetFloatField(router, jfield_GeneralRouter_slightTurn);
	rConfig->router->shortWaySlightTurn = ienv->GetFloatField(router, jfield_GeneralRouter_shortWaySlightTurn);
	rConfig->router->roundaboutTurn = ienv->GetFloatField(router, jfield_GeneralRouter_roundaboutTurn);
	rConfig->router->shortWayRoundaboutTurn = ienv->GetFloatField(router, jfield_GeneralRouter_shortWayRoundaboutTurn);
